...
## 技术栈：
  react + redux + webpack + react-router + ES6/7/8 + immutable + scss


## 运行项目（nodejs 6.0+）

```
 git clone https://github.com/wlx200510/react-wlx.git
...
>  开发环境 windows10  Chrome 63  nodejs 8.9.1

>  fork from [bailicangdu](https://github.com/bailicangdu/react-pxq)
...
在此补充一下withRouter的用法，这个在网络上搜有好多不对的，这个特性是用于向组件内注入三个路由变量(match、location、history),这三个变量原本只是由Route组件上的component属性对应的组件可以在props中获取到，但有了withRouter的包裹方法，就可以在任意需要的组件中进行history的相关API操作，以及获取location.params变量等，可以说是相当方便，要注意的是withRouter并没有组件懒加载的功能，它的功能一定要分清楚，这里我在header中使用了withRouter注入，不过目前暂时没有用途，只是在这里说明一下适用情况。
